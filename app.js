...
// --- SCROLL DETECTION (Cache le menu quand on descend) ---
let lastScrollTop = 0;
...
window.addEventListener('scroll', function() {
    ...
    // Si on descend et qu'on n'est pas tout en haut -> Cacher
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    renderHistory();
    renderCalendar();
    // initSortable(); // Si besoin du drag & drop

    // Restauration de la séance en cours (si crash ou fermeture)
    if (savedSession) {
        try {
            const sessionState = JSON.parse(savedSession);
            const select = document.getElementById('selectProgram');
            // Vérifie si le programme existe toujours
            if (select.querySelector(`option[value="${sessionState.prog}"]`)) {
                select.value = sessionState.prog;
                currentProgramKey = sessionState.prog;
                chargerInterface(false); // false = ne pas effacer les logs existants
                
                // Remet les valeurs dans les inputs
                ...
                // Remet les logs validés
                if (sessionState.logs) {
                    currentSessionLogs = sessionState.logs;
                    // Marquer les boutons comme validés si besoin (optionnel, complexe à refaire parfaitement)
                }
            }
...
// --- LOGIQUE PRINCIPALE (SÉANCE) ---

function hasSessionData() {
    // Vérifie si l'utilisateur a commencé à entrer des trucs
...
function handleProgramChange() {
    ...
    // Protection anti-fausse manip
    if (hasSessionData()) {
        if (!confirm("Tu as des données en cours. Changer de séance effacera tout. Continuer ?")) {
            select.value = currentProgramKey; // On remet l'ancien
            return;
        }
    }
    currentProgramKey = newKey;
    chargerInterface(true); // true = on reset tout pour la nouvelle séance
}
...
function chargerInterface(shouldClear = true) { 
    ...
    // Boucle pour afficher les exercices
    for(let i = 0; i < exos.length; i++) { 
        ...
        if(exoA.isSuperset && exoB && exoB.isSuperset) { 
            ...
            i++; // On saute le prochain car déjà traité
        } else { 
...
function createDropInput(className, unit) { 
    return `<div class="input-wrapper"><input type="number" class="${className}" placeholder="" min="0" oninput="if(this.value!=='')this.value=Math.abs(this.value)"><span class="unit-label">${unit}</span></div>`; 
}
...
function renderSuperset(container, exoA, idxA, exoB, idxB, progName) { 
    ...
    // Style inline pour forcer la transparence (au cas où le CSS bug)
...
// --- GESTION DES REPS/POIDS & DÉGRESSIVES ---
...
function ajouterDegressive(baseId, exoName, progName, setNum) { 
    ...
    div.className = 'input-row drop-row'; 
...
function getDropsString(containerId) { 
    ...
    container.querySelectorAll('.drop-row').forEach(row => { 
        const w = row.querySelector('.drop-weight').value; 
        const r = row.querySelector('.drop-reps').value; 
...
function validerExerciceNormal(nomExo, idx, totalSets, btn) { 
    // Si déjà validé -> On déverrouille pour modifier
    if (btn.classList.contains('validated')) { 
        btn.classList.remove('validated'); btn.innerText = "Exercice Fini"; 
        for(let s=1; s<=totalSets; s++) { 
            document.getElementById(`p_${idx}_${s}`).disabled = false; 
            document.getElementById(`r_${idx}_${s}`).disabled = false; 
            const container = document.getElementById(`container_${idx}_${s}`); 
            container.querySelectorAll('input').forEach(i => i.disabled = false); 
            if(container.querySelector('.btn-mini-add')) container.querySelector('.btn-mini-add').style.display = 'flex'; 
            container.querySelectorAll('.drop-icon').forEach(icon => icon.style.display = 'block'); 
        } 
    ...
    for(let s=1; s<=totalSets; s++) { 
        const p = document.getElementById(`p_${idx}_${s}`).value; 
        const r = document.getElementById(`r_${idx}_${s}`).value; 
        if(p && r) { 
            let dropText = getDropsString(`container_${idx}_${s}`); 
            tempLogs.push({ exo: nomExo, perf: `${p} kg x ${r} reps${dropText}`, serie: s }); 
            savedCount++; 
        } 
    } 
    
    if(savedCount > 0) { 
        currentSessionLogs.push(...tempLogs); 
        btn.classList.add('validated'); btn.innerText = "Validé"; 
        for(let s=1; s<=totalSets; s++) { 
            document.getElementById(`p_${idx}_${s}`).disabled = true; 
            document.getElementById(`r_${idx}_${s}`).disabled = true; 
            const container = document.getElementById(`container_${idx}_${s}`); 
            container.querySelectorAll('input').forEach(i => i.disabled = true); 
            if(container.querySelector('.btn-mini-add')) container.querySelector('.btn-mini-add').style.display = 'none'; 
            container.querySelectorAll('.drop-icon').forEach(icon => icon.style.display = 'none'); 
        } 
        saveCurrentSessionState(); 
    } else { 
        alert("Remplis au moins une série !"); 
    } 
}
...
function validerSuperset(nomA, idxA, nomB, idxB, totalSets, btn) { 
    if (btn.classList.contains('validated')) { 
        ...
        for(let s=1; s<=totalSets; s++) { 
            if(document.getElementById(`p_${idxA}_${s}`)) { 
                document.getElementById(`p_${idxA}_${s}`).disabled = false; 
                document.getElementById(`r_${idxA}_${s}`).disabled = false; 
                // ... déverrouillage A
            }
            if(document.getElementById(`p_${idxB}_${s}`)) { 
                document.getElementById(`p_${idxB}_${s}`).disabled = false; 
                document.getElementById(`r_${idxB}_${s}`).disabled = false; 
                // ... déverrouillage B
            }
            // Code simplifié pour la lisibilité, l'idée est de tout réactiver
            const cA = document.getElementById(`container_${idxA}_${s}`); if(cA) cA.querySelectorAll('input').forEach(i=>i.disabled=false);
            const cB = document.getElementById(`container_${idxB}_${s}`); if(cB) cB.querySelectorAll('input').forEach(i=>i.disabled=false);
        } 
    ...
    for(let s=1; s<=totalSets; s++) { 
        if(document.getElementById(`p_${idxA}_${s}`)) { 
            const pA = document.getElementById(`p_${idxA}_${s}`).value; 
            const rA = document.getElementById(`r_${idxA}_${s}`).value; 
            if(pA && rA) { 
                let dropTextA = getDropsString(`container_${idxA}_${s}`); 
                tempLogs.push({ exo: nomA, perf: `${pA} kg x ${rA} reps${dropTextA}`, serie: s }); 
                savedCount++; 
            } 
        }
        if(document.getElementById(`p_${idxB}_${s}`)) { 
            const pB = document.getElementById(`p_${idxB}_${s}`).value; 
            const rB = document.getElementById(`r_${idxB}_${s}`).value; 
            if(pB && rB) { 
                let dropTextB = getDropsString(`container_${idxB}_${s}`); 
                tempLogs.push({ exo: nomB, perf: `${pB} kg x ${rB} reps${dropTextB}`, serie: s }); 
                savedCount++; 
            } 
        }
    } 
    ...
    if(savedCount > 0) { 
        ...
        for(let s=1; s<=totalSets; s++) { 
             const cA = document.getElementById(`container_${idxA}_${s}`); if(cA) cA.querySelectorAll('input').forEach(i=>i.disabled=true);
             const cB = document.getElementById(`container_${idxB}_${s}`); if(cB) cB.querySelectorAll('input').forEach(i=>i.disabled=true);
        } 
        saveCurrentSessionState();
    } else { 
        alert("Remplis au moins une série !"); 
...
function validerSupersetBuilder() {
    ...
    let sA = parseInt(document.getElementById('buildSeries').value); if(sA < 1 || isNaN(sA)) sA = 1; 
    const rA = document.getElementById('buildReps').value;
    
    const nB = document.getElementById('buildExoNameB').value; 
    let sB = parseInt(document.getElementById('buildSeriesB').value); if(sB < 1 || isNaN(sB)) sB = 1; 
...
function renderBuilder() { 
    ...
    for (let i = 0; i < tempBuilderList.length; i++) { 
        ...
        let editingClass = ''; 
        if (currentEditingIndex !== -1) { 
            if (i === currentEditingIndex || (item.isSuperset && i === currentEditingIndex + 1)) { 
        ...
        if (item.isSuperset && tempBuilderList[i+1] && tempBuilderList[i+1].isSuperset) { 
            ...
            listDiv.innerHTML += `<div class="builder-item builder-item-superset ${editingClass}" data-json='${dataJson}'><span class="delete-x" onclick="tempBuilderList.splice(${i}, 2); resetBuilderForm(); renderBuilder()">✖</span><div class="builder-click-zone" onclick="editBuilderItem(${i})" title="Cliquer pour modifier"><div style="margin-bottom:12px;"> <span class="builder-exo-name">${item.name}</span> <span class="builder-exo-info">${item.sets} x ${item.reps} reps</span> </div><div> <span class="builder-exo-name">${nextItem.name}</span> <span class="builder-exo-info">${nextItem.sets} x ${nextItem.reps} reps</span> </div></div></div>`; 
            i++; 
        } else { 
...
function updateSelectMenu() { 
    ...
    s.innerHTML = '<option value="" disabled selected>Choisir une Séance</option>'; 
    ...
    Object.keys(DB.progs).forEach(k => { 
        ...
        for (let i = 0; i < exos.length; i++) { 
            ...
            if (e.isSuperset && exos[i+1] && exos[i+1].isSuperset) { 
                ...
                html += ` <div class="prog-line prog-line-superset"> <span class="prog-line-name">${e.name}</span> <span class="prog-line-info">${e.sets} x ${e.reps} reps</span> </div>`; 
                html += ` <div class="prog-line prog-line-superset prog-line-superset-b"> <span class="prog-line-name">${eNext.name}</span> <span class="prog-line-info">${eNext.sets} x ${eNext.reps} reps</span> </div>`; 
                html += `</div>`; 
                i++; 
            } else { 
...
function renderHistory() { 
    ...
    if (historyState.view === 'categories') { 
        ...
        filtered.forEach(session => { 
            ...
            wrapper.className = 'hist-session'; 
...
function resetHistoryOnly() { 
    if(confirm("Effacer tout l'historique ?")) { 
        DB.history = []; 
        localStorage.setItem('gym_v21_history', JSON.stringify([])); 
        historyState.view = 'categories'; 
        renderHistory(); 
    } 
}
